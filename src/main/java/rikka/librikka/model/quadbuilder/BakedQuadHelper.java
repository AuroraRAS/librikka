...
public class BakedQuadHelper {
    ...
    public static BakedQuad bake(
        ...
        BakedQuad quad = new BakedQuad(Ints.concat(
                ...
                BakedQuadHelper.vertexToInts(x4, y4, z4, Color.WHITE.getRGB(), texture, u4, v4, normal)), 0, side,
                texture, true);
    ...
    public static int[] vertexToInts(float x, float y, float z, int color, TextureAtlasSprite texture, float u, float v,
            int normal) {
        // 将 0~16 的传统坐标除以 16，转换为现代 Minecraft 期望的 0.0f~1.0f 标准化坐标
        ...
        return new int[] {
                ...
                color,
                Float.floatToRawIntBits(texture.getU(normalizedU)),
                Float.floatToRawIntBits(texture.getV(normalizedV)),
                1,
                normal
        };
    }
